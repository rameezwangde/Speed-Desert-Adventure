...
export default function BuggyGuide() {
  ...
  return <section className="guide-section">
    <div className="guide-intro"><SectionHeading eyebrow="PREPARE FOR THE TERRAIN" title="Inclusive Guide For" accent="Sand Dune Buggy Dubai" /><p>A practical tour guide structure covering preparation, safety and the desert driving experience.</p></div>
    <div className="guide-visual"><img src="/guide-teal.png" alt="Teal dune buggy overlooking the UAE desert" loading="lazy"/>{['SAFETY','WHAT TO WEAR','DRIVING','AGE REQUIREMENT','DESERT CONDITIONS','PREPARATION'].map((x,i)=><span className={`marker marker-${i+1}`} key={x}>{x}</span>)}</div>
